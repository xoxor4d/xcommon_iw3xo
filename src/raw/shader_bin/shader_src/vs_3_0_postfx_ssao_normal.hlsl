...
struct PixelInput
{
	...
	//float4x4 InvProjMat : TEXCOORD1;
	
	/* float4 invProj0		: TEXCOORD1;
	float4 invProj1		: TEXCOORD2;
	float4 invProj2		: TEXCOORD3;
	float4 invProj3		: TEXCOORD4; */
};
...
PixelInput vs_main( const VertexInput vertex ) 
{
	...
	//pixel.InvProjMat = inverseViewMatrix;//inverse(projectionMatrix);

	/* pixel.invProj0 = invProjMatrix[0];
	pixel.invProj1 = invProjMatrix[1];
	pixel.invProj2 = invProjMatrix[2];
	pixel.invProj3 = invProjMatrix[3];  */
