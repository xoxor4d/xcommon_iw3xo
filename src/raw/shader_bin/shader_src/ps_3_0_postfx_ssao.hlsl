...
PixelOutput ps_main( const PixelInput pixel )
{
    ...
    float _NUMSTEPS       = 8.0f; // xo_ssao_numSteps  // 8.0  // not in use, not avail. on PS3.0
    float _NUMDIR         = 8.0f; // xo_ssao_numDir    // 16.0 // not in use, not avail. on PS3.0
    ...
    else
    {
        ...
        float3 dPdv = min_diff(P, Pt, Pb) * (realRenderTargetSize.y * rcpres.x);
        ...
        fragment.color = float4(finalAO, finalAO, finalAO, 1.0f );
    }
